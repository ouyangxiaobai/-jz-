...
$(function(){
    ...
    getData();
   	...
   	$("#addbutton").on("click",function(){
   		...
   		window.location.href=getIP()+"media/saomayongche/saomayongcheAdd.jsp";
   	})
    ...
    $("#delbutton").on("click",function(){
    	...
    	if(confirm("确定要删除吗？")){
        	...
        	del.ajax(getIP()+"saomayongche.action?delMore",function(data){
        		if(data.success){
    ...
    function getData(){  
	   	...
	   	getPageData(sel,getIP()+"vrenyuan.action?list",function(data){
	   		...
	   		if(data.success){
				...
				if(len >0){
					$.each(data.rows,function(i,val){
						...
						var ddd="<a onclick='update(\""+val.id+"\")' class='btn btn-info'>预约</a>";
						...
						html+="<tr>" +
								...
								"<td>"+val.leixing+"</td>" +  
								"<td>"+val.a3+"</td>" +  
								"<td><img style='width:85px;' src="+getIP()+val.a5+"></img></td>" +
								"<td>"+
								ddd+
...
function update(id){
	 ...
	 window.location.href=getIP()+"media/saomayongche/saomayongcheModify.jsp?eid="+id;
}
//修改
function jieshu(id){
